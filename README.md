...
# past the 400K retrieval line, with the bucket and the pace meter both up
O5 1M M | 412k ████████▶ | 53m 71% · 5d18h 40% · F 21% · ████░░░░ | 1h5m +$0.25 $14.20
...
- **Session signals** *(conditional)* — `!N` in red counts self-reported slips (`[slip]` markers), `~N` counts final messages the punctuation-glue Stop hook flagged (yellow for one, orange from two). Each hides at zero. Both are read from a per-session counts file that the claude-config hooks write, so the segment is empty when those hooks are not installed.
- **Rate limits** *(when available)* — 5-hour and 7-day usage as `<time-to-reset> <percent>%`, separated by `·`. The 7-day limit is followed by a **bidirectional gas-pedal pace meter** (hidden when you're on pace, so it only appears when it matters): it fills **left in red** when you're burning too fast (you'll hit the cap before the window resets), and — if you opt in with `STATUSLINE_PACE_SHOW_COLD=true` — **right in blue** when you're burning too slow (you'll leave weekly budget unused, which is lost at reset). It's time-aware: a steady off-pace burn stays near-empty early (still time to correct) and only fills as it gets urgent near the reset. The cold/under-use side is off by default. Per-model weekly buckets sit after the last plan percentage present and before that meter, dot-separated from it, one field each as `<initial> <percent>%` with no bar of their own: `F 4%` is the `Current week (Fable)` row from `/usage`. Those buckets are not in the statusline payload, so they are read from Claude Code's own cached usage response in `~/.claude.json`. No network call is made. Claude Code refreshes that cache only on a successful usage fetch, and there is no background poll, so for interactive use that means running `/usage`. The field therefore carries the reading from your last `/usage` run. It hides itself an hour on, which is where Claude Code's own reader discards the block. Running `/usage` brings it back.
...
| `STATUSLINE_PACE_GAMMA` | 1.5 | Pace-meter response curve: `1` linear, higher keeps it flatter until urgent. |
...
| `STATUSLINE_MODEL_USAGE_MAX_AGE` | 3600 | Seconds of age on Claude Code's cached usage reading past which the per-model field hides itself. Matches Claude Code's own read cutoff. |
