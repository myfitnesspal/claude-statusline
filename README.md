...
- **Rate limits** *(when available)* — 5-hour and 7-day usage as `<time-to-reset> <percent>%`, separated by `·`. The 7-day limit is followed by a **bidirectional gas-pedal pace meter** (hidden when you're on pace, so it only appears when it matters): it fills **left in red** when you're burning too fast (you'll hit the cap before the window resets) and **right in blue** when you're burning too slow (you'll leave weekly budget unused — it's lost at reset). It's time-aware: a steady off-pace burn stays near-empty early (still time to correct) and only fills as it gets urgent near the reset. Weekly budget doesn't roll over, so both directions matter.
...
| `STATUSLINE_PACE_SHOW_ON_PACE` | false | By default the pace meter is hidden when you're on pace (shows only when hot or cold). Set `true` to always render it. |
