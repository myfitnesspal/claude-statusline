...
- **Rate limits** *(when available)* — 5-hour and 7-day usage as `<time-to-reset> <percent>%`, separated by `·`. The 7-day limit is followed by a **bidirectional gas-pedal pace meter** (hidden when you're on pace, so it only appears when it matters): it fills **left in red** when you're burning too fast (you'll hit the cap before the window resets), and — if you opt in with `STATUSLINE_PACE_SHOW_COLD=true` — **right in blue** when you're burning too slow (you'll leave weekly budget unused, which is lost at reset). It's time-aware: a steady off-pace burn stays near-empty early (still time to correct) and only fills as it gets urgent near the reset. The cold/under-use side is off by default. Per-model weekly buckets follow, one field each as `<initial> <percent>% <bar>`: `F 4% ░░░░░░░░` is the `Current week (Fable)` row from `/usage`. Those buckets are not in the statusline payload, so `model-usage-refresh.sh` fetches them out of band into a cache; the field hides itself when the cache is missing, empty, or stale. Note that the usage endpoint is rate limited per account on an hours-scale window, and Claude Code's own `/usage` polling spends the same budget, so these buckets refresh on the order of hours rather than minutes; a `429` response is recorded and respected rather than retried.
...
| `STATUSLINE_MODEL_USAGE_TTL` | 900 | Seconds since the last fetch attempt before a render refreshes the per-model buckets again. A `retry-after` from the server takes precedence. |
...
| `STATUSLINE_MODEL_USAGE_MAX_AGE` | 21600 | Seconds of cached-data age past which the per-model field hides itself instead of showing a stale number. |
